...
class ConnectionManager:
    ...
    async def connect(self, doc_id: str, user_id: str, user_name: str, websocket: WebSocket) -> UserPresence:
        ...
        stale_user_ids = [
            uid for uid, (_, pres) in self.active_connections[doc_id].items()
            if pres.user_name == user_name
        ]
        for uid in stale_user_ids:
            old_ws, _ = self.active_connections[doc_id].pop(uid)
            try:
                await old_ws.close()
            except Exception:
                pass
        ...
        used_colors = {
            pres.color for _, pres in self.active_connections[doc_id].values()
        }
        color = None
        for c in COLOR_PALETTE:
            if c not in used_colors:
                ...
                break
        if not color:
            hash_idx = sum(ord(ch) for ch in user_name) % len(COLOR_PALETTE)
            color = COLOR_PALETTE[hash_idx]
    ...
    async def send_personal(self, doc_id: str, user_id: str, message: dict):
        if doc_id in self.active_connections and user_id in self.active_connections[doc_id]:
            ...
            try:
                ...
                self.disconnect(doc_id, user_id)
